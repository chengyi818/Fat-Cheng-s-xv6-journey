...
## JOS中是如何处理这些问题的?
### wakeup丢失
1. JOS是单处理器OS,并且在内核态禁用中断.因此`wakeup`不会在线程sleep完成前,偷偷执行.

### 终止睡眠线程
1. JOS只有少量简单的系统调用
2. JOS没有disk驱动,因此没有复杂的必须原子完成的操作.
3. 仅有一个需要注意的调用`IPC recv()`,当`env_destroy()`在执行时,目标进程一定不在执行.目标进程在执行`recv()`时,依然可以被安全销毁.

## 总结
1. `sleep/wait`提供了可以让进程间协作的功能
2. 并发以及中断使得我们不得不考虑`wakeup`丢失的问题
